...
public class gift_item_proxy  {
	...
	public gift_item_proxy(Context  con)
	{
		context=con;
	}
	...
	public void sendRequest(final ServerListener listener) {
		...
		final int max =get_jrla_gift_list_max();
	...
	private List<Map<String, Object>> createData(int num3, int num4) {
		...
		String str = get_jrla_gift_list_data(num3, num4); // getdate(num3,num4);
		if (str == "null") {
			return list;
		} else {
...
//				map.put("url", data[6]);
	...
	private int get_jrla_gift_list_max() {
		return communicate.get_max_table(communicate.table_jrla_gift_list,
				"");

	}

	private String get_jrla_gift_list_data(int da_start, int da_end) {
		return communicate.get_data_table(communicate.table_jrla_gift_list,
